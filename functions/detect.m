function [resstruct,feat_pyramid] = detect(I, models, params)
    ...
    %
    % NOTE: These local detections can be pooled with pool_exemplars_dets.m
    %
    ...
    % params: Localization parameters (see get_default_params.m)
    %
    ...
    [rs1, t1] = detectdriver(I, models, params);
    rs1 = prune_nms(rs1, params);

    if doflip == 1
        params.detect_add_flip = 1;
        [rs2, t2] = detectdriver(I, models, params);
    ...
    for q = 1:length(rs1.bbs)
        rs1.xs{q} = cat(2,rs1.xs{q}, rs2.xs{q});
...
function [resstruct,t] = detectdriver(I, models, params)

    if ~isfield(params,'max_models_before_block_method')
        params.max_models_before_block_method = 20;
    end

    if (length(models) > params.max_models_before_block_method) || (~isempty(params.nnmode))
        [resstruct,t] = detectdriverBLOCK(I, models, params);
        return;
    end
    ...
    elseif ~isfield(models{1}.model,'init_params')
        ...
        fprintf(1,['Warning: not all exemplars have save sbin, using first]\n']);
        sbin = models{1}.model.init_params.sbin;
    end
    ...
    for level = length(t.hog):-1:1
        ...
        rmsizes = cellfun2(@(x)size(x), rootmatch);

        for exid = 1:N
            if prod(rmsizes{exid}) == 0
                fprintf(fileid, 'prod = 0\n');
                continue
            end
            ...
            [uus,vvs] = ind2sub(rmsizes{exid}(1:2), indexes);

            scale = t.scales(level);

            o = [uus vvs] - t.padder;

            bbs = ([o(:,2) o(:,1) o(:,2)+size(ws{exid},2) o(:,1)+size(ws{exid},1)] - 1) * sbin/scale + 1 + repmat([0 0 -1 -1],length(uus),1);
            ...
            if params.detect_save_features == 1
                for z = 1:NKEEP
                    xs{exid}{end+1} = reshape(t.hog{level}(uus(z)+(1:sss(1))-1, vvs(z)+(1:sss(2))-1,:), [],1);
                end
            end
...
function [resstruct,t] = detectdriverBLOCK(I, models, params)
    ...
    for i = 1:length(models)
        ...
        t(1:models{i}.model.hg_size(1),1:models{i}.model.hg_size(2),:) = models{i}.model.w;
        templates(:,:,:,i) = t;
        template_masks(:,:,:,i) = repmat(double(sum(t.^2,3)>0),[1 1 fsize]);

        if (~isempty(params.nnmode)) || (isfield(params,'wtype') && strcmp(params.wtype,'dfun')==1)
            x = zeros(S(1),S(2),fsize);
            x(1:models{i}.model.hg_size(1),1:models{i}.model.hg_size(2),:) = reshape(models{i}.model.x(:,1),models{i}.model.hg_size);
            templates_x(:,:,:,i) = x;
        end
    end
    ...
    elseif isempty(params.nnmode)
        fprintf(fileid, 'params.nnmode is empty\n');
    ...
    elseif strcmp(params.nnmode,'normalizedhog') == 1
        fprintf(fileid, 'params.nnmode = normalizedhog\n');
        r = exemplar_matrix' * X;
    elseif strcmp(params.nnmode,'nndfun') == 1
        fprintf(fileid, 'params.nnmode = nndfun\n');
    ...
    for exid = 1:N
        ...
        if isempty(goods)
            fprintf(fileid, 'good is empty\n');
            continue
        end
        [sorted_scores,bb] = psort(-r(exid,goods)', min(params.detect_max_windows_per_exemplar, length(goods)));
        bb = goods(bb);
        ...
        bbs = ([o(:,2) o(:,1) o(:,2)+size(ws{exid},2) o(:,1)+size(ws{exid},1)] - 1) .* repmat(sbin./scales',1,4) + 1 + repmat([0 0 -1 -1],length(scales),1);
        bbs(:,5:12) = 0;
        bbs(:,5) = (1:size(bbs,1));
        bbs(:,6) = exid;
        bbs(:,8) = scales;
        ...
        if (params.detect_add_flip == 1)
            fprintf(fileid, 'params.detect_add_flip = 1\n');
            bbs = flip_box(bbs,t.size);
            bbs(:,7) = 1;
        end
    ...
    if params.detect_save_features == 0
        fprintf(fileid, 'params.detect_save_features = 0\n');
        resstruct.xs = cell(N,1);
    end
    fclose(fileid);
end
...
function rs = prune_nms(rs, params)
    % Prunes redundant detections using non maximum suppression.
    
    if ~isfield(params,'detect_exemplar_nms_os_threshold') || (params.detect_exemplar_nms_os_threshold >= 1) || (params.detect_exemplar_nms_os_threshold == 0)
        return;
    end
    ...
    if ~isempty(rs.xs)
        fprintf(fileid, 'rs.xs is not empty\n');
...
function t = get_pyramid(I, ~, params)
    % Extract feature pyramid from variable I (which could be either an 
    % image or already a feature pyramid)
    ...
    if isnumeric(I)
        ...
        % Compute pyramid
        [t.hog, t.scales] = pyramid(I, params);
